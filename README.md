...
- **도서관 / AI 요약**: 실제 백엔드(`/api/materials`)와 연동 완료. 도서관 화면 헤더의 업로드 버튼(또는 "내 자료" 목록의 업로드 버튼)으로 자료를 올리고 선택할 수 있으며, AI 요약 화면은 선택한 자료의 실제 `ai_summary`/`key_concepts`를 조회해 표시합니다 (처리 중이면 자동 폴링).
- **복습하기 / AI 질문**: 아직 화면 시안용 더미 데이터(하드코딩)입니다.

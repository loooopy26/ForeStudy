...
2. **환경 변수** — `backend/.env` (`.env.example` 참고, git에는 올라가지 않음)
   ```
   UPSTAGE_API_KEY=up_...
   PUBLIC_DATA_SERVICE_KEY=공공데이터포털에서_발급받은_일반_인증키
   ...
   `PUBLIC_DATA_SERVICE_KEY`는 진행 중인 자격증의 시험 일정·응시 수수료 조회에 필요합니다. [한국산업인력공단 국가기술자격 종목별 시험정보](https://www.data.go.kr/data/15003029/openapi.do) 활용신청 후 발급된 일반 인증키(Decoding)를 입력하세요.

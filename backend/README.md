...
1. **PostgreSQL + pgvector**
   ...
   - `db/schema.sql`을 실행한 지 오래된 로컬 DB는 이후 추가된 컬럼(예: `quizzes.quiz_type`)이 없을 수 있습니다. `db.py`의 `_run_startup_migrations`가 첫 요청 시 자동으로 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`를 실행해 맞춰주므로 보통은 수동 조치가 필요 없지만, 배치고사 생성(`POST /api/materials/{id}/quiz`)이 `UndefinedColumnError`로 실패한다면 서버를 껐다가 다시 켜서 마이그레이션이 실행되게 하세요.
...
2. **환경 변수** — `backend/.env` (`.env.example` 참고, git에는 올라가지 않음)
   ...
   `TMAP_APP_KEY`는 [SK Open API Network](https://openapi.sk.com)에서 발급받는 앱 키로, 위치 기반 학습장소 추천/시험 당일 어시스턴트(`/api/location/*`)에 필요합니다. 없어도 서버는 정상 기동되고, 해당 엔드포인트 호출 시에만 503 에러로 안내합니다.
   `GOOGLE_MAPS_API_KEY`를 추가하면 대중교통 경로는 [Google Routes API](https://developers.google.com/maps/documentation/routes)로 조회합니다. TMAP은 POI 검색·도보·자동차 경로에 계속 사용됩니다.
...
| **위치(TMAP)** | GET | `/api/location/health` | TMAP 설정 상태 확인 (`tmap_configured`) | **실제 동작** |
| **위치(TMAP)** | POST | `/api/location/nearby-study-places` | 현재 위치 기준 주변 학습장소(도서관/스터디카페 등) 검색 + 도보/자동차/대중교통 거리·이동시간. `query`(자연어, 예: "가까운 공부하기 좋은 카페 알려줘")로 검색어를 대신할 수 있다 — LLM이 있으면 의도 분석, 없으면 규칙 기반 키워드 매칭으로 대체 | **실제 동작** (Q-NET 미사용, 프론트 미연결) |
| **위치(TMAP)** | POST | `/api/location/exam-day-assistant` | 요청 body로 입력한 시험장 주소를 좌표 변환 후 TMAP·백엔드가 이동시간/거리/권장 출발시각을 계산. LLM은 이 계산값과 주변 카페·식당·프린트 후보를 입력받아 추천 이동수단/리스크/행동계획/준비물(`guidance`)만 생성(계산은 하지 않음) — LLM이 없거나 실패하면 규칙 기반 안내로 대체 | **실제 동작** (Q-NET 미사용, 프론트 미연결) |
...
│   └── location.py         # TMAP 위치 기능 (실제 동작, 프론트 미연결)
├── services/
...
│   └── tmap.py              # TMAP POI 검색/지오코딩/경로(도보·자동차·대중교통) 클라이언트

...
- 오직 물체 자체의 색상·재질·형태·디테일만 묘사하세요. 물체가 놓인 장소·배경·환경(거리,
  방, 실외, 자연, 하늘, 바닥, 그림자 등)은 단 한 단어도 언급하지 마세요 — "차량"처럼
  현실적인 소재일수록 "주차된", "도로 위의", "화창한 거리의" 같은 장면 묘사를 덧붙이고
  싶어지는데, 그러면 코드가 뒤에 붙이는 'plain white background' 지시를 덮어써서 배경이
  같이 그려집니다. 물체 하나만 진공 상태에 떠 있듯 묘사하세요.
  (나쁜 예: "a red sports car parked on a sunlit street" — "parked on a sunlit street" 삭제.
   좋은 예: "a sleek red sports car with glossy paint and aerodynamic curves")
...
STYLE_SUFFIX = (
    ...
    "thick clean outline, centered composition, isolated object floating with no "
    "scenery, no environment, no ground, no shadow, plain white background, "

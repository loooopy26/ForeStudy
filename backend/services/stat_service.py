...
로그인 계정(UUID) 기준 Postgres study_sessions/quiz_attempts/curriculum_days로 계산한다
(예전에는 타이머는 SQLite 데모 유저, 퀴즈/플랜 지표는 항상 Postgres 데모 유저를 봐서
로그인 계정과 무관하게 모두가 같은 수치를 봤다)."""
...
async def get_user_stats(user_id: str, material_id: str | None = None) -> dict:
    pool = await get_pool()
    sessions = await pool.fetch(
        """
        SELECT s.id, s.ended_at, s.studied_minutes, s.max_uninterrupted_minutes,
               (SELECT count(*) FROM study_session_interruptions i
                WHERE i.study_session_id = s.id) AS interruption_count
        FROM study_sessions s
        WHERE s.user_id = $1 AND s.status = 'completed'
          AND ($2::uuid IS NULL OR s.study_material_id = $2::uuid)
    ...
    total_study_minutes = sum(row["studied_minutes"] for row in sessions)
    current_streak_days = _current_streak_days_from_sessions(sessions)
    focus = _calculate_focus(sessions)
    quiz_metrics = await _get_quiz_metrics(pool, user_id, material_id)
    comprehension = _calculate_comprehension(quiz_metrics)
    plan_metrics = await _get_plan_completion_metrics(pool, user_id, sessions, material_id)
...
def _calculate_focus(sessions: list[asyncpg.Record]) -> int:
    ...
    for session in recent_sessions:
        ...
        interruption_penalty = max(0, session["interruption_count"] - 1) * 8
...
async def _get_quiz_metrics(pool: asyncpg.Pool, user_id: str, material_id: str | None = None) -> dict:
...
async def _get_plan_completion_metrics(
    pool: asyncpg.Pool, user_id: str, sessions: list[asyncpg.Record], material_id: str | None = None
) -> dict:
...
def _current_streak_days_from_sessions(sessions: list[asyncpg.Record]) -> int:
    days = {row["ended_at"].date() for row in sessions if row["ended_at"] is not None}
    if not days:
        return 0

...
# 세마포어(동시 호출 수 3)로 게이트웨이 앞에서 대기하는 시간은 이 안에 포함되지 않지만,
# 장소 10곳 x 최대 3모드가 세마포어 하나를 나눠 쓰다 보니 대기가 길어지는 경우가 있어
# 10초는 종종 부족했다(실측: walk/car/transit 각각 간헐적 타임아웃 발생). 여유를 좀 더 둔다.
...
class TmapPermissionError(TmapRequestError):
    """앱 키에 해당 상품(예: 대중교통 길안내)이 활성화되어 있지 않을 때 (403).
    SK Open API 콘솔에서 상품을 활성화해야 하는 문제라 재시도로 해결되지 않는다."""
...
async def _request(client: httpx.AsyncClient, method: str, url: str, **kwargs) -> dict:
    ...
    if resp.status_code == 403:
        logger.error(
            "TMAP API 권한 오류(403) %s: %s — 앱 키에 해당 상품이 SK Open API 콘솔에서 "
            "활성화되어 있는지 확인하세요(예: 대중교통 길안내). 재시도로 해결되지 않습니다.",
            url,
            resp.text[:500],
        )
        raise TmapPermissionError(
            f"TMAP API가 403(권한 없음) 응답을 반환했습니다: {resp.text[:300]}"
        )
...
# 앱 키에 상품이 활성화되어 있지 않은 모드(403)는 콘솔에서 사람이 활성화하기 전까지는
# 재시도해도 계속 실패한다. 매 요청마다 그 모드를 다시 호출하면 세마포어 슬롯만 낭비하고
# (주변 학습장소 추천처럼 장소당 여러 모드를 한꺼번에 호출하는 경우 특히 영향이 크다)
# 정작 성공 가능한 walk/car 호출의 대기 시간만 늘어난다. 한 번 403을 확인한 모드는
# 아래 TTL 동안 실제 호출 없이 바로 None을 반환하고, TTL이 지나면 콘솔 설정이 바뀌었을 수
# 있으니 다시 한번 확인해본다.
_PERMISSION_DISABLED_UNTIL: dict[str, float] = {}
_PERMISSION_DISABLE_SECONDS = 600.0
...
async def get_routes_for_modes(origin: dict, destination: dict, modes: list[str]) -> dict:
    ...
    그대로 올려 호출자가 한 번에 처리하게 한다. 특정 모드가 403(권한 없음)이면 해당 모드를
    일정 시간 비활성화해 불필요한 재호출을 막는다."""
    _require_app_key()
    ...
    async def _safe(mode: str):
        ...
        disabled_until = _PERMISSION_DISABLED_UNTIL.get(mode)
        if disabled_until is not None and time.monotonic() < disabled_until:
            logger.debug("%s 경로 조회 건너뜀 (최근 403 확인, 콘솔에서 상품 활성화 필요)", mode)
            return mode, None

        try:
            result = await func(origin, destination)
        except TmapPermissionError as exc:
            logger.warning(
                "%s 경로를 %.0f초 동안 건너뜁니다 (권한 없음): %s", mode, _PERMISSION_DISABLE_SECONDS, exc
            )
            _PERMISSION_DISABLED_UNTIL[mode] = time.monotonic() + _PERMISSION_DISABLE_SECONDS
            return mode, None
            ...
            _PERMISSION_DISABLED_UNTIL.pop(mode, None)
            return mode, result
